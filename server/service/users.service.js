...
const addUser = async ({ firstName, lastName, email, userId, password, city, street, role }) => {
    try {

        const newUser = new User({ firstName, lastName, email, userId, password: createHashedPassword(password), city, street, role });
...
const updateUser = async (id, data) => {
    try {
        const result = await User.updateOne({ _id: mongoose.Types.ObjectId(id) }, {
            $set: data
        });
...
module.exports = {
    addUser, updateUser, deleteUser, fetchUsers, fetchUsersId,
